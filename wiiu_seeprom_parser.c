...
{
	printf("\n=======================================\n");
	printf("   Wii U SEEPROM parser v1.1   \n");
	printf("=======================================\n\n");
	...
	FILE * seeprom_file;
	if (argc < 2) {
		seeprom_file = fopen("seeprom.bin", "rb");
	} else {
	...
	if (argc > 2) {
		...
		otp_file = fopen("otp.bin", "rb");
	}
	if (otp_file != NULL) {
		//Load otp file
		printf("Reading otp...\n");
		if (fread(otp, 1, 0x400, otp_file) != 0x400 && argc > 2) {
	...
	if (use_otp == 1) {
		...
		print_hex("\n\nDecrypting boot parameters with key ", wiiu_seeprom_key, 0x10);
		...
		printf   ("Boot parameters for boot0 about boot1:\n");
		...
		printf   ("Boot parameters for boot0 about boot1 copy:\n");
